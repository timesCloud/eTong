...
//代码创建初始化
-(id)initWithFrame:(CGRect)frame withLineWidth:(CGFloat)width{
...
-(void)draw_jishu:(CGRect)rect{
    if(rect.size.width>rect.size.height){
        _led_jishu.frame = CGRectMake(0, 0, 2*rect.size.height/3, 3*rect.size.height/5);
    }else {
        _led_jishu.frame = CGRectMake(0, 0, 2*rect.size.width/3, 3*rect.size.width/5);
    }
