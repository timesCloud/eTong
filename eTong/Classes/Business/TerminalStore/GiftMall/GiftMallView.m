...
@interface GiftMallView()<UITableViewDelegate, UITableViewDataSource>
...
- (void)loadDataByClearAll:(BOOL)needClearAll withComplete:(void(^)())complete{
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (!error)
        {
            ...
            for (Gift *gift in objects) {
                [cellObjectArray addObject:gift];
            }
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    return kGiftCellHeight;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    static NSString *vCellIdentify = @"GiftCell";
    GiftTableViewCell *vCell = [tableView dequeueReusableCellWithIdentifier:vCellIdentify];
    if (vCell == nil) {
        vCell = [[GiftTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:vCellIdentify];
    }
    
    vCell.selectionStyle = UITableViewCellSelectionStyleNone;
    [vCell setArticle:[cellObjectArray objectAtIndex:indexPath.row]];
